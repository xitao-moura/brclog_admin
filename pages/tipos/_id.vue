...
export default {
    ...
    data() {
        return {
            ...
            items: [{
                ...
                {
                    text: `tipo id: ${this.$route.params.id}`,
                    active: true,
                },
            ],
            tipo: {
    ...
    methods: {
        async getTipo(){
            let doc = await firebase.firestore().collection('tipos').doc(this.$route.params.id).get()
            this.tipo = doc.data()
            this.tipo.id = doc.id
        },
        async updateTipo(){
            ...
            this.$v.$touch()
            if (!this.$v.$invalid) {
                await firebase.firestore().collection('tipos').doc(this.tipo.id).update(this.tipo)
                this.$router.push({path: '/tipos'})
            }
        },
        voltar(){
            this.$router.go(-1)
